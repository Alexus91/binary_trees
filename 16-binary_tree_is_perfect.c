...
{
	binary_tree_t *left_ht, *right_ht;
	...
	if (left_ht == NULL || right_ht == NULL)
		return (0);
	if (binary_tree_height(left_ht) == binary_tree_height(right_ht))
	{
		if (binary_tree_is_perfect(left_ht) && binary_tree_is_perfect(right_ht))
			return (1);
	}
	return (0);
}
